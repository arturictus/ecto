defmodule Ecto.ChangesetTest do
  ...
  test "get_assoc/3" do
    ...
    assert get_assoc(base_changesset, :comments, :changeset) == [
             %{comment_changeset | action: :update}
           ]
  ...
  test "validate_length/3 with map" do
    changeset =
      changeset(%{"seo_metadata" => %{"keywords" => ["foo", "bar"], "slug" => "my-post-1"}})
      |> validate_length(:seo_metadata, min: 2, max: 3)
    ...
    changeset =
      changeset(%{"seo_metadata" => %{"keywords" => ["foo", "bar"], "slug" => "my-post-2"}})
      |> validate_length(:seo_metadata, min: 2, max: 2)

    assert changeset.valid?

    changeset =
      changeset(%{"seo_metadata" => %{"keywords" => ["foo", "bar"], "slug" => "my-post-3"}})
      |> validate_length(:seo_metadata, is: 2)

    assert changeset.valid?

    changeset =
      changeset(%{"seo_metadata" => %{"keywords" => ["foo", "bar"], "slug" => "my-post-4"}})
      |> validate_length(:seo_metadata, min: 3)
    ...
    assert changeset.errors == [
             seo_metadata:
               {"should have at least %{count} item(s)",
                count: 3, validation: :length, kind: :min, type: :map}
           ]
    ...
    changeset =
      ...
      |> validate_length(:seo_metadata, max: 2)
    ...
    assert changeset.errors == [
             seo_metadata:
               {"should have at most %{count} item(s)",
                count: 2, validation: :length, kind: :max, type: :map}
           ]
    ...
    changeset =
      ...
      |> validate_length(:seo_metadata, is: 10)
    ...
    assert changeset.errors == [
             seo_metadata:
               {"should have %{count} item(s)",
                count: 10, validation: :length, kind: :is, type: :map}
           ]
    ...
    changeset =
      ...
      |> validate_length(:seo_metadata, is: 10, message: "yada")

    assert changeset.errors == [
             seo_metadata: {"yada", count: 10, validation: :length, kind: :is, type: :map}
           ]
  end
  ...
  test "validate_number/3 with bad value" do
    assert_raise ArgumentError,
                 "expected field `virtual` to be a decimal, integer, or float, got: \"Oops\"",
                 fn ->
  ...
  test "validate_number/3 with bad target" do
    # Number value
    assert_raise ArgumentError,
                 "expected option `greater_than` to be a decimal, integer, or float, got: 0..10",
                 fn ->
    ...
    assert_raise ArgumentError,
                 "expected option `greater_than` to be a decimal, integer, or float, got: 0..10",
                 fn ->
                   validate_number(changeset(%{"decimal" => Decimal.new(11)}), :decimal,
                     greater_than: 0..10
                   )
                 end
  end
  ...
  describe "unsafe_validate_unique/4" do
    ...
    test "accepts a prefix option" do
      ...
      assert_receive [
        MockRepo,
        function: :exists?,
        query: %Ecto.Query{prefix: "my_prefix"},
        opts: []
      ]
    end
    ...
    test "accepts nulls_distinct option" do
      ...
      assert Macro.to_string(check_expr) ==
               "true and is_nil(&0.body()) and is_nil(&0.title()) and &0.color() == ^0"
    end
  ...
  test "unique_constraint/3" do
    ...
    assert_raise ArgumentError, ~r/invalid match type: :invalid/, fn ->
      change(%Post{})
      |> unique_constraint(:title, name: :whatever, match: :invalid, message: "match is invalid")
    end
  end
  ...
  test "assoc_constraint/3" do
    ...
    assert_raise ArgumentError, ~r/invalid match type: :invalid/, fn ->
      change(%Comment{})
      |> assoc_constraint(:post, name: :whatever, match: :invalid, message: "match is invalid")
    end
  end
